...
bool test_is_tail() {
  ...
  printf("%s\n", "test_is_tail now have tests.");
  game_state_t* state = create_default_state();
  if (!assert_true("test is-tail", is_tail(get_board_at(state, 4, 4)))){
    return false;
  }
  if (!assert_true("test is-tail", !is_tail(get_board_at(state, 4, 5)))){
    return false;
  }
...
bool test_is_snake() {
  // TODO: Implement this if you'd like!
  printf("%s\n", "test_is_snake now have tests.");
  game_state_t* state = create_default_state();
  if (!assert_true("test is_snake", is_snake(get_board_at(state, 4, 4)))){
    return false;
  }
  if (!assert_true("test is_snake", !is_snake(get_board_at(state, 4, 5)))){
    return false;
  }
  if (!assert_true("test is_snake", is_snake(get_board_at(state, 5, 4)))){
    return false;
  }
  return true;
}
...
bool test_body_to_tail() {
  // TODO: Implement this if you'd like!
  printf("%s\n", "test_body_to_tail now have tests.");
  game_state_t* state = create_default_state();
  if (!assert_equals_char("test body_to_tail", 'd', body_to_tail(get_board_at(state, 5, 4)))){
    return false;
  }
  return true;
}
...
bool test_incr_x() {
  // TODO: Implement this if you'd like!
  printf("%s\n", "test_incr_x now have tests.");
  game_state_t* state = create_default_state();

  if (!assert_equals_int("test incr_x", 1, incr_x(get_board_at(state, 4, 4)))){
    return false;
  } 
  if (!assert_equals_int("test incr_x", 0, incr_x(get_board_at(state, 3, 3)))){
    return false;
  }
  return true;
}
...
bool test_incr_y() {
  // TODO: Implement this if you'd like!
  printf("%s\n", "test_incr_y now have tests.");
  game_state_t* state = create_default_state();

  if (!assert_equals_int("test incr_y", 0, incr_y(get_board_at(state, 4, 4)))){
    return false;
  } 
  if (!assert_equals_int("test incr_y", 0, incr_y(get_board_at(state, 3, 3)))){
    return false;
  }
  return true;
}
